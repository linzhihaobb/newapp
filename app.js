  ...
  (req, res, next) => {
    ...
    fs.exists(`public/upload/product/${id}`, function (exists) {
      if (exists) {
        for (let i in files) {
          var file = files[i];
          fs.rename(file.path, "public/upload/product/" + id + "/" + file.originalname, function (err) {
            if (err) {
              throw err;
            }
          });
    ...
    for (let i in files) {
      ...
      var fileInfo = {};
      var style = {};
      style.backgroundImage = "url(/upload/product/" + id + "/" + file.originalname + ")";
...
// 轮播图修改后上传
app.post("/api/v2/edit/:id", uploadMulti.single("slideshow"),function(req,res){
  var data = req.body
  var file = req.file;
  var filename;
  filename = file.originalname;
...
app.post('/api/v1/edit/:id', uploadMulti.array('img', 3), function (req, res) {
  ...
  fs.exists(`public/upload/product/${req.params.id}`, function (exists) {
    if (exists) {
      for (let i in files) {
        var file = files[i];
        fs.rename(file.path, "public/upload/product/" + req.params.id + "/" + file.originalname, function (err) {
          if (err) {
            throw err;
          }
        });
  ...
  for (let i in files) {
    ...
    var fileInfo = {};
    var style = {};
    style.backgroundImage = "url(/upload/product/" + req.params.id + "/" + file.originalname + ")";
    style.backgroundSize = "100%";
    style.backgroundRepeat = "no-repeat";
    style.backgroundPosition = "center";
    fileInfo.style = style;
  ...
  data.imgSrc = "upload/product/" + req.params.id + "/" + files[0].originalname;
